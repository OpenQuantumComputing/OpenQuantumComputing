...
class QAOAMaxKCutOnehot(QAOABase):
    ...
    def binstringToLabels_MaxKCut(self, k_cuts, num_V, binstring):
        ...
        label_list = [
            int(binstring[j * k_bits : (j + 1) * k_bits], 2) for j in range(num_V)
        ]
        label_string = ""
    ...
    def cost(self, string):
        ...
        for edge in G.edges():
            ...
            j = int(edge[1])
            li = min(
                k_cuts - 1, int(labels[i])
            )  ## e.g. for k_cuts=3, labels 2 and 3 should be equal
            lj = min(
                k_cuts - 1, int(labels[j])
            )  ## e.g. for k_cuts=3, labels 2 and 3 should be equal
            if li != lj:
    ...
    def createCircuit(self, angles, depth):
        ...
        if version == 1:
            if k_cuts == 2:
                ...
                raise Exception(
                    "Circuit creation for k=",
                    k_cuts,
                    " not implemented for version 1 (hard coded).",
                )
            ...
            if version == 1:
                ...
                if k_cuts == 2:
                    ...
                    for edge in G.edges():
                        ...
                        Cn_U3_0theta0(
                            circ,
                            [J - 1 + ind for ind in range(1, k_bits)],
                            J + k_bits - 1,
                            -wg,
                        )
                    ...
                    for edge in G.edges():
                        ...
                        for fi in range(n):
                            for fj in range(n):
                                ...
                                Cn_U3_0theta0(
                                    circ, [ind_a1, ind_a2], J + k_bits - 1, -wg
                                )
                                circ.mcx([J, J + 1, J + 2], ind_a2)
                                circ.mcx([I, I + 1, I + 2], ind_a1)
                                itt = -1
                    ...
                    raise Exception(
                        "Circuit creation for k=",
                        k_cuts,
                        " not implemented for version 2 (decomposed).",
                    )
