        ...
        while hasnotrun:
            ...
            try:
                ...
                if "Error code: 3458" in str(e):
                    ...
                    print(
                        "{j} Error! Code: {c}, Message: {m}, Time {t}".format(
                            j=str(filename),
                            c=type(e).__name__,
                            m=str(e),
...
def decompose3_IZ(H):
    ...
    for i in [0, 3]:
        for j in [0, 3]:
            for k in [0, 3]:
                a = (
                    HilbertSchmidtInnerProduct(np.kron(S[i], np.kron(S[j], S[k])), H)
                    / 2**3
                )
                if not np.isclose(a, 0):
                ...
                for l in [0, 3]:
                    a = (
                        HilbertSchmidtInnerProduct(
                            np.kron(S[i], np.kron(S[j], np.kron(S[k], S[l]))), H
                        )
                        / 2**4
                    )
                    if not np.isclose(a, 0):
    ...
    for i in [0, 3]:
        for j in [0, 3]:
            for k in [0, 3]:
                for l in [0, 3]:
                    for m in [0, 3]:
                        a = HilbertSchmidtInnerProduct(
                            np.kron(
                                S[i], np.kron(S[j], np.kron(S[k], np.kron(S[l], S[m])))
                            ),
                            H,
                        )
                        # /2**6
                            ...
                            a = HilbertSchmidtInnerProduct(
                                np.kron(
                                    ...
                                    np.kron(
                                        S[j],
                                        np.kron(
                                            S[k], np.kron(S[l], np.kron(S[m], S[n]))
                                        ),
                                    ),
                                ),
                                H,
